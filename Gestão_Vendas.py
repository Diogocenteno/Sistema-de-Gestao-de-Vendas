...
# Autor: Mutante
# Data: 19-09-2025
# Versão: 2.31 (com Importação Inteligente e Correção de Erros)
...
# --- Classe da Calculadora de Preço (Adaptada para ser um Frame) ---
class CalculadoraPrecoVenda(ttk.Frame):
    ...
    def __init__(self, parent, style):
        ...
        # --- Entradas de Dados ---
        input_frame = ttk.LabelFrame(self, text=" 1. Insira os Dados do Produto ", padding="15")
        input_frame.pack(fill=X, pady=5)
        
        self.entry_nome_produto = self.create_entry_field(
            input_frame, 
            "Nome do Produto:", 
            "Nome ou descrição do item para salvar no histórico.",
            validate=False
        )
        self.entry_custo_produto = self.create_entry_field(
            input_frame, 
            "Custo do Produto (R$):", 
            "Custo de aquisição ou produção do item."
        )
        self.entry_gasto_operacional = self.create_entry_field(
            input_frame, 
            "Gasto Operacional (R$):",
            "Custos fixos e variáveis diluídos por unidade (ex: aluguel, marketing, etc.)."
        )
        self.entry_impostos_pct = self.create_entry_field(
            input_frame,
            "Alíquota de Imposto (%):",
            "Percentual de imposto sobre a venda (ex: Simples Nacional)."
        )
        self.entry_custo_transacao = self.create_entry_field(
            input_frame, 
            "Taxa da Transação (%):",
            "Taxa da maquininha de cartão ou gateway de pagamento (opcional)."
        )
        self.entry_margem_lucro = self.create_entry_field(
            input_frame, 
            "Lucro Desejado (%):",
            "Percentual de lucro que deseja obter sobre o preço de venda."
        )
        ...
        # --- Resultados ---
        result_frame = ttk.LabelFrame(self, text=" 2. Resultados do Cálculo ", padding="15")
        result_frame.pack(fill=X, pady=5)
        
        self.preco_ideal_var = tk.StringVar(value="R$ 0,00")
        self.lucro_estimado_var = tk.StringVar(value="R$ 0,00")

        self.create_result_line(result_frame, "Lucro Bruto Estimado:", self.lucro_estimado_var, self.copy_to_clipboard, ("Helvetica", 12), bootstyle="info")
        self.create_result_line(result_frame, "Preço Ideal de Venda:", self.preco_ideal_var, self.copy_to_clipboard, ("Helvetica", 24, "bold"), bootstyle="success", pady_top=15)

        # --- Detalhamento do Preço (inicialmente vazio) ---
        self.details_frame = ttk.LabelFrame(self, text=" 3. Detalhamento do Preço de Venda ", padding="15")
    ...
    def create_entry_field(self, parent, label_text, tooltip_text, validate=True):
        ...
        if validate:
            entry = ttk.Entry(frame, width=30, validate='key', validatecommand=self.vcmd)
        else:
    ...
    def calcular_preco(self):
        try:
            ...
            total_percentual = margem_lucro_pct + custo_transacao_pct + impostos_pct
            ...
            custo_total = custo_produto + gasto_operacional
            denominador = 1 - (total_percentual / 100)
            preco_venda_ideal = custo_total / denominador
            
            imposto_valor = preco_venda_ideal * (impostos_pct / 100)
            custo_transacao_valor = preco_venda_ideal * (custo_transacao_pct / 100)
            ...
            messagebox.showerror("Por favor, insira valores numéricos válidos.", "Erro de Entrada", parent=self)
    ...
    def salvar_excel(self):
        if self.preco_ideal_var.get() == "R$ 0,00":
        ...
        headers = [
            ...
            "Preço de Venda (R$)", "Lucro Bruto (R$)"
        ]

        dados = [
            datetime.now().strftime("%d-%m-%Y %H:%M:%S"),
            self.entry_nome_produto.get() or "Não informado",
            self.entry_custo_produto.get().replace(".", ","),
            self.entry_gasto_operacional.get().replace(".", ","),
            self.entry_impostos_pct.get().replace(".", ","),
            self.entry_custo_transacao.get().replace(".", ",") or "0",
            self.entry_margem_lucro.get().replace(".", ","),
            self.preco_ideal_var.get().replace("R$ ", ""),
            self.lucro_estimado_var.get().replace("R$ ", "")
        ]
...
class SalesApp:
    ...
    def _create_widgets(self):
        ...
        tab_calculadora = ttk.Frame(notebook, padding=10)
    ...
    def _create_vendas_tab(self, parent_tab):
        """Cria todos os widgets da aba de gestão de vendas."""
